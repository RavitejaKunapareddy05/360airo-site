...
import { Label } from '@/components/ui/label';
import { 
  CheckCircle2, Mail, Zap, Calendar, Flame, Building, Globe,
  Phone, User, Clock, CheckCircle, Filter, ArrowUpRight,
...
// ✅ OUTSIDE component — loaded once at module level
...
const toUtcIsoFromSelection = (date: Date, hour: number, minute: number, timezone: 'us' | 'india') => {
  const offsetMinutes = timezone === 'us' ? -5 * 60 : 5 * 60 + 30;
...
export default function DemoFormPage() {
  const [formData, setFormData] = useState({
    ...
    companyName: '', userCount: '', specificNeeds: ''
  });
  const [loading, setLoading] = useState(false);
  const [status, setStatus] = useState('');
  const [formSubmitted, setFormSubmitted] = useState(false);
  ...
  const generateCalendar = (date: Date) => {
    ...
    while (currentDate <= endDate) {
      ...
      const isToday =
        ...
        dayDate.getFullYear() === today.getFullYear();
      const isAvailable = dayDate > today && isCurrentMonth;
      days.push({ date: dayDate, day: dayDate.getDate(), month: dayDate.getMonth(),
        year: dayDate.getFullYear(), isCurrentMonth, isToday, available: isAvailable });
      currentDate.setDate(currentDate.getDate() + 1);
    }
    setCalendarDays(days);
  };
  ...
  const convertUSToIndiaTime = (usHour: number, usMinute: number): string => {
    let indiaHour = usHour + 10;
    const indiaMinute = usMinute + 30;
    let finalIndiaHour = indiaHour;
    let finalIndiaMinute = indiaMinute;
    if (indiaMinute >= 60) { finalIndiaHour += 1; finalIndiaMinute -= 60; }
    if (finalIndiaHour >= 24) { finalIndiaHour -= 24; }
    const ampm = finalIndiaHour >= 12 ? 'PM' : 'AM';
    const displayHour = finalIndiaHour > 12 ? finalIndiaHour - 12 : finalIndiaHour === 0 ? 12 : finalIndiaHour;
    return `${displayHour}:${finalIndiaMinute.toString().padStart(2, '0')} ${ampm}`;
  };
  ...
  const generateTimeSlots = () => {
    ...
    for (let hour = 9; hour <= 20; hour++) {
      const usHour12 = hour > 12 ? hour - 12 : hour;
      const usAmpm = hour >= 12 ? 'PM' : 'AM';
      const usFormatted = `${usHour12}:00 ${usAmpm} ET`;
      const indiaFormatted = convertUSToIndiaTime(hour, 0) + ' IST';
      slots.push({
        usTime: `${hour.toString().padStart(2, '0')}:00`,
        usFormatted,
        indiaTime: `${hour + 10}:0`,
        indiaFormatted,
        value: `${hour}:0`
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setStatus('');
    try {
      setTempFormData({ ...formData, submittedAt: new Date().toLocaleString() });
      setStatus('Successfully submitted.');
      setShowConfetti(true);
      setTimeout(() => {
        setFormSubmitted(true);
  ...
  const handleDateSelect = (day: CalendarDay) => {
    if (day.available) { setSelectedDate(day.date); setSelectedTime(null); }
  };

  const handleTimeSelect = (time: string) => setSelectedTime(time);

  const handleScheduleConfirm = async () => {
    if (!selectedDate || !selectedTime || !tempFormData) {
      setStatus('Please select both date and time.');
      return;
    }
    setLoading(true);
    setStatus('Scheduling your demo...');
    try {
      const formattedDate = selectedDate.toLocaleDateString('en-US', {
        weekday: 'long', year: 'numeric', month: 'long', day: 'numeric'
      });
      const [hour, minuteRaw] = selectedTime.split(':').map(Number);
      ...
      const formattedTime = selectedTimezone === 'us' ? timeSlot.usFormatted : timeSlot.indiaFormatted;
      const timeZoneText = selectedTimezone === 'us' ? 'US Eastern Time (ET)' : 'India Standard Time (IST)';
      const startIsoUtc = toUtcIsoFromSelection(selectedDate, hour, minute, selectedTimezone);
      const nextDay = new Date(selectedDate);
      nextDay.setDate(nextDay.getDate() + 1);
      const alternativeDate = nextDay.toLocaleDateString('en-US', {
        weekday: 'long', year: 'numeric', month: 'long', day: 'numeric'
      ...
      const schedulePayload: Record<string, string> = {
        ...
        lastName: tempFormData.lastName,
        email: tempFormData.email,
        phone: tempFormData.phone || 'Not provided',
        companyName: tempFormData.companyName,
        userCount: tempFormData.userCount,
        specificNeeds: tempFormData.specificNeeds || 'Not specified',
        ...
        submittedAt: tempFormData.submittedAt || new Date().toLocaleString(),
        scheduleSubmittedAt: new Date().toLocaleString()
      };

      const calendarRes = await fetch('/api/demo/schedule', {
        method: 'POST',
      ...
      if (!calendarRes.ok || !calendarJson?.ok) {
        throw new Error(
          calendarJson?.error ||
          calendarJson?.details?.error?.message ||
          'Calendar sync failed'
        );
      }

      // ✅ USER template — snake_case keys match {{to_email}}, {{first_name}} etc.
      const userTemplateParams = {
        to_email: schedulePayload.email,
        name: '360airo Team',
        reply_to: '360airo@gmail.com',
        first_name: schedulePayload.firstName,
        last_name: schedulePayload.lastName,
        preferred_date: schedulePayload.preferredDate,
        preferred_time: schedulePayload.preferredTime,
        time_zone: schedulePayload.timeZone,
        demo_duration: '15 minutes',
        support_email: 'support@360airo.com'
        // No meeting_link — not sent to client
      };

      // ✅ ADMIN template — camelCase keys match {{firstName}}, {{meetingLink}} etc.
      const adminTemplateParams = {
        name: 'Demo Bot - 360airo',
        to_email: '360airo@gmail.com',
        reply_to: schedulePayload.email,
        firstName: schedulePayload.firstName,
        lastName: schedulePayload.lastName,
        email: schedulePayload.email,
        phone: schedulePayload.phone,
        companyName: schedulePayload.companyName,
        userCount: schedulePayload.userCount,
        specificNeeds: schedulePayload.specificNeeds,
        preferredDate: schedulePayload.preferredDate,
        alternativeDate,
        preferredTime: schedulePayload.preferredTime,
        timeZone: schedulePayload.timeZone,
        submittedAt: schedulePayload.submittedAt,
        submitted_at: schedulePayload.submittedAt,
        scheduleSubmittedAt: schedulePayload.scheduleSubmittedAt,
        title: `Demo Scheduled: ${schedulePayload.companyName}`
      };

      try {
        await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_USER_TEMPLATE_ID, userTemplateParams, EMAILJS_PUBLIC_KEY);
      } catch (err: any) {
        throw new Error(`User template failed (${err?.status || 'unknown'}): ${err?.text || 'EmailJS error'}`);
      }
      try {
        await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_ADMIN_TEMPLATE_ID, adminTemplateParams, EMAILJS_PUBLIC_KEY);
      } catch (err: any) {
        throw new Error(`Admin template failed (${err?.status || 'unknown'}): ${err?.text || 'EmailJS error'}`);
      }

      setShowConfetti(true);
      setStatus('Demo scheduled. Check your email for confirmation.');
      setTimeout(() => {
        setShowCalendar(false);
        setFormData({ firstName: '', lastName: '', email: '', phone: '', companyName: '', userCount: '', specificNeeds: '' });
        setTempFormData(null);
        setShowConfetti(false);
      }, 2000);
      ...
      console.error('Scheduling Error:', error);
      setStatus(error?.message || 'Scheduling failed. Please try again or contact support.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const userCountOptions = [
    { value: '1-5', label: '1-5 Users' }, { value: '6-10', label: '6-10 Users' },
    { value: '11-25', label: '11-25 Users' }, { value: '26-50', label: '26-50 Users' },
    { value: '51-100', label: '51-100 Users' }, { value: '101-250', label: '101-250 Users' },
    { value: '251-500', label: '251-500 Users' }, { value: '501-1000', label: '501-1,000 Users' },
    { value: '1000+', label: '1,000+ Users' }
  ];
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {[...Array(20)].map((_, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: -100 }}
              animate={{ opacity: [0, 0.4, 0], y: [0, window.innerHeight], x: Math.sin(i * 0.5) * 100 }}
              transition={{ duration: 5 + Math.random() * 5, repeat: Infinity, delay: i * 0.3, ease: 'linear' }}
              className="absolute w-1 h-1 rounded-full bg-[#FF8A65]/30"
              style={{ left: `${Math.random() * 100}%` }}
            />
          ))}
        ...
        <section className="relative pt-32 pb-20 px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
              ...
              <h1 className="text-4xl sm:text-5xl font-bold text-white leading-tight mb-4">
                <span className="block mb-2">Schedule Your Demo</span>
                <span className="block bg-gradient-to-r from-[#FF6432] via-[#FF8A65] to-white bg-clip-text text-transparent">Select Date & Time</span>
              </h1>
	              <div className="h-1 bg-gradient-to-r from-[#FF6432] via-[#FF8A65] to-[#FFB74D] rounded-full max-w-md mx-auto mb-6" />
	              <p className="text-xl text-white/80 max-w-2xl mx-auto">Choose a convenient time for your personalized 15-minute demo</p>
	              <div className="mt-6 max-w-3xl mx-auto text-left bg-white/5 border border-white/15 rounded-xl p-5">
	                <h3 className="text-white font-bold text-lg mb-2">What Is Calendar Sync?</h3>
	                <p className="text-white/80 leading-relaxed">
	                  Calendar sync links your demo booking with external calendars like Google Calendar to check
	                  availability, prevent double-bookings, auto-adjust time zones, and keep reminders and event
	                  details aligned for everyone.
	                </p>
	              </div>
	            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mb-8">
              <div className="bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-white/10 max-w-md mx-auto">
                <div className="flex items-center justify-center gap-6">
                  <button onClick={() => setSelectedTimezone('us')}
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all ${selectedTimezone === 'us' ? 'bg-gradient-to-r from-[#FF6432] to-[#FF8A65] text-white' : 'text-white/70 hover:text-white hover:bg-white/5'}`}>
                    <Globe className="h-4 w-4" /><span>US Eastern Time</span>
                  </button>
                  <button onClick={() => setSelectedTimezone('india')}
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all ${selectedTimezone === 'india' ? 'bg-gradient-to-r from-[#FF6432] to-[#FF8A65] text-white' : 'text-white/70 hover:text-white hover:bg-white/5'}`}>
                    <Globe className="h-4 w-4" /><span>India Standard Time</span>
                  </button>
                </div>
                <p className="text-white/60 text-sm text-center mt-3">
                  {selectedTimezone === 'us' ? 'Showing times in US Eastern (converted to IST)' : 'Showing times in India (converted to ET)'}
                </p>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }}>
              <Card className="bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-sm border border-white/20 rounded-2xl shadow-2xl overflow-hidden">
                <CardHeader className="pb-6 bg-gradient-to-r from-[#FF6432]/10 to-[#FF8A65]/10 border-b border-white/10">
                  <div className="flex items-center justify-between">
                    <div>
                      <CardTitle className="text-2xl font-bold text-white">Select Demo Time</CardTitle>
                      <CardDescription className="text-white/80">Available time slots for the next 30 days</CardDescription>
                    </div>
                    <div className="flex items-center gap-4">
                ...
                <CardContent className="pt-8">
                  <div className="grid lg:grid-cols-2 gap-8">
                      ...
                      <div className="grid grid-cols-7 gap-2">
                        {calendarDays.map((day, index) => (
                          <motion.button key={index}
                            ...
                            onClick={() => handleDateSelect(day)}
                            disabled={!day.available}
                            className={`relative h-12 rounded-lg flex items-center justify-center text-sm font-medium transition-all duration-200
                              ${!day.isCurrentMonth ? 'opacity-30' : ''}
                              ${day.isToday ? 'border-2 border-[#FF8A65]' : ''}
                              ${selectedDate?.getDate() === day.date.getDate() && selectedDate?.getMonth() === day.date.getMonth()
                                ? 'bg-gradient-to-r from-[#FF6432] to-[#FF8A65] text-white'
                                : day.available ? 'bg-white/5 text-white hover:bg-white/10' : 'bg-white/5 text-white/40 cursor-not-allowed'
                              }`}
                          >
                            {day.day}
                            {day.isToday && (
                    ...
                    <div>
                      ...
                      {selectedDate ? (
                        <div className="space-y-3">
                          <div className="text-white/70 mb-4">Select a time for your 15-minute demo session</div>
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 max-h-[300px] overflow-y-auto pr-2">
                            {timeSlots.map((slot, index) => (
                          ...
                          {selectedTime && (
                            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-6">
                              <div className="bg-gradient-to-r from-[#FF6432]/10 to-[#FF8A65]/10 rounded-xl p-6 border border-[#FF8A65]/30">
                                ...
                                <div className="space-y-3 mb-6">
                                  <div className="flex items-center gap-3 text-white/80">
                                    <Clock className="h-4 w-4" /><span>Duration: 15 minutes</span>
                                  </div>
                                  <div className="flex items-center gap-3 text-white/80">
                                    <Video className="h-4 w-4" /><span>Our team will reach out with meeting details</span>
                                  </div>
                                </div>
                                <Button onClick={handleScheduleConfirm} disabled={loading}
                                  className="w-full bg-gradient-to-r from-[#FF6432] to-[#FF8A65] hover:opacity-90 text-white h-12 text-lg font-semibold">
                                  {loading ? (
                                    <div className="flex items-center justify-center gap-3">
                                ...
                                {status && (
                                  <div className={`mt-4 p-3 rounded-lg text-center text-sm font-medium ${
                                    status.toLowerCase().includes('failed') || status.toLowerCase().includes('please')
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="text-center mt-8">
              <Button variant="outline" onClick={() => setShowCalendar(false)} className="border-white/20 text-white hover:bg-white/10">
                <ArrowUpRight className="h-4 w-4 mr-2 rotate-180" /> Back to Form
              </Button>
            </motion.div>
          </div>
        </section>
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#0a0014] via-[#19001d] to-[#0a0014] relative overflow-hidden">
        {showConfetti && (
          <Confetti width={window.innerWidth} height={window.innerHeight}
            recycle={false} numberOfPieces={500} gravity={0.15}
            colors={['#FF6432', '#FF8A65', '#FFB74D', '#FFFFFF']} />
        )}
        <section className="relative pt-32 pb-20 px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl mx-auto text-center">
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="mb-12">
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
                ...
                <div className="space-y-6 text-white/90">
                  <p className="text-lg">Thank you for scheduling your demo! Here are your session details:</p>
                  <div className="bg-gradient-to-r from-[#FF6432]/10 to-[#FF8A65]/10 rounded-xl p-6 max-w-md mx-auto">
                    <div className="space-y-4">
                      <div className="flex items-center justify-center gap-3">
                        <Calendar className="h-6 w-6 text-[#FF8A65]" />
                  ...
                  <div className="bg-white/5 rounded-xl p-6">
                    ...
                    <ul className="space-y-3 text-white/80 text-left">
                      <li className="flex items-start">
                        <CheckCircle2 className="h-5 w-5 text-[#FF8A65] mt-0.5 mr-3 flex-shrink-0" />
                        <span>Our team will reach out with the meeting details</span>
                      </li>
                      <li className="flex items-start">
                        <CheckCircle2 className="h-5 w-5 text-[#FF8A65] mt-0.5 mr-3 flex-shrink-0" />
                        <span>Add the meeting to your calendar</span>
                  ...
                  <p className="text-lg">
                    ...
                    <span className="text-[#FF8A65] font-semibold">{tempFormData?.email}</span>
                  </p>
                </div>
              </motion.div>
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="space-y-6">
                <Button onClick={() => {
                  setFormSubmitted(false); setSelectedDate(null); setSelectedTime(null);
                  setShowCalendar(false); setSelectedTimezone('us'); setTempFormData(null); setShowConfetti(false);
                }} className="bg-gradient-to-r from-[#FF6432] to-[#FF8A65] hover:opacity-90 text-white px-8 py-6 text-lg font-semibold">
                  <Calendar className="mr-3" /> Schedule Another Demo
                </Button>
              </motion.div>
            </motion.div>
          </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="inline-block mb-6">
              <span className="inline-flex items-center px-6 py-3 rounded-full bg-white/10 backdrop-blur-sm border border-[#FF6432]/50 text-white font-semibold text-sm">
                <Flame className="h-4 w-4 text-[#FF8A65] mr-3" /> Get Your Personalized Demo
              </span>
            </motion.div>
            <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
              className="text-4xl sm:text-5xl lg:text-6xl font-black text-white leading-tight mb-4">
              <span className="block">Get Your</span>
              <span className="block bg-gradient-to-r from-[#FF6432] via-[#FF8A65] to-white bg-clip-text text-transparent">Free AI Email Demo</span>
            </motion.h1>
          ...
          <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="max-w-3xl mx-auto">
            <Card className="bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-sm border border-white/20 rounded-2xl shadow-2xl overflow-hidden">
              <CardHeader className="pb-6 bg-gradient-to-r from-[#FF6432]/10 to-[#FF8A65]/10 border-b border-white/10">
                <CardTitle className="text-3xl font-bold text-white text-center">Request Your Demo</CardTitle>
                <CardDescription className="text-white/80 text-center text-lg">Complete the form to schedule your personalized demo</CardDescription>
              </CardHeader>
              <CardContent className="pt-8">
                <form onSubmit={handleSubmit} className="space-y-8">
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="space-y-6">
                    <h3 className="text-xl font-bold text-white flex items-center">
                      <User className="w-5 h-5 mr-2 text-[#FF8A65]" /> Basic Information
                    </h3>
                    <div className="grid gap-6 md:grid-cols-2">
                      <div className="space-y-3">
                        <Label htmlFor="firstName" className="text-white font-semibold">First Name *</Label>
                        <Input id="firstName" type="text" placeholder="John" value={formData.firstName}
                          onChange={(e) => setFormData({ ...formData, firstName: e.target.value })} required
                          className="bg-white/5 border-white/20 text-white h-12 rounded-lg focus:border-[#FF6432] focus:ring-[#FF6432] placeholder-white/40" />
                      </div>
                      <div className="space-y-3">
                        <Label htmlFor="lastName" className="text-white font-semibold">Last Name *</Label>
                        <Input id="lastName" type="text" placeholder="Doe" value={formData.lastName}
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="space-y-6">
                    <h3 className="text-xl font-bold text-white flex items-center">
                      <Building className="w-5 h-5 mr-2 text-[#FF8A65]" /> Company Information
                    </h3>
                    <div className="space-y-3">
                      <Label htmlFor="companyName" className="text-white font-semibold">Company Name *</Label>
                      <Input id="companyName" type="text" placeholder="Your Company Inc." value={formData.companyName}
                    ...
                    <div className="grid gap-6">
                      <div className="space-y-3">
                        <Label htmlFor="userCount" className="text-white font-semibold">Number of Users *</Label>
                        <Select value={formData.userCount} onValueChange={(value) => setFormData({ ...formData, userCount: value })}>
                          <SelectTrigger className="bg-white/5 border-white/20 text-white h-12 rounded-lg">
                            <SelectValue placeholder="Select number of users" />
                          </SelectTrigger>
                          <SelectContent className="bg-[#0a0014] border-white/20 text-white">
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }} className="space-y-6">
                    <Button type="submit"
                      className="w-full h-14 text-lg font-bold rounded-lg bg-gradient-to-r from-[#FF6432] to-[#FF8A65] hover:opacity-90 transition-all hover:scale-[1.02] text-white relative overflow-hidden group"
                      disabled={loading}>
                      <span className="relative z-10 flex items-center justify-center">
                    ...
                    {status && (
                      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                        className={`p-4 rounded-lg text-center font-medium ${
                          status.includes('✅') ? 'bg-green-500/20 text-green-400 border border-green-500/30'
                          : status.includes('❌') ? 'bg-red-500/20 text-red-400 border border-red-500/30'
